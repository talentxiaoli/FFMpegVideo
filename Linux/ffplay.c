...
static int fill_iobuffer(void * opaque,uint8_t *buf, int bufsize){
	...
	if(!feof(fp_open)){
        ...
        int true_size = 512 * 320 * 3 / 2;
        // printf("true_size = %d\n", true_size);
        printf("shdata frameIndex = %d\n", shdata->frameIndex);
        printf("buffSize = %d\n", bufsize);
        memcpy(buf, shdata->data, bufsize);
		return true_size;
	}else{
		return -1;
	}
...
{
    printf("xiaolizi2\n");
    fp_open = fopen("/home/lihai/Desktop/xiaoli666/test/test.yuv", "rb");
    if (!fp_open) {
        printf("mmmmmmm failed\n");
        return 0;
    }
    // fclose(fp_open);
    VideoState *is = arg;
...
static int initSm() {
    ...
    int shmid = -1;
   //1 创建共享内存
    ret = shmget(1234, sizeof(shared_data), 0666 | IPC_CREAT);
    if (ret == -1) {
        perror("创建共享内存失败\n");
        return -1;
    }
    ...
    // if (ret == -1) {
    //     perror("delete share memory failed\n");
    //     return -1;
    // }
...
int main(int argc, char **argv)
{
    ...
    SDL_CreateThread(readSm, "readSm", NULL);
